...
//  The last column of your CSV will be used as the classification.
    ...
    private static HashMap<String, ArrayList<Double>> points1 = new HashMap<>();
        ...
        addButt.setOnAction(new EventHandler<ActionEvent>() {
            ...
            public void handle(ActionEvent event) {
                ...
                for (int i = 0; i < 2000; i+=200) {
                    ...
                    runInstruc = "--RS: " + restarts + " HL " + hiddenLayer + " Iter " + i
                    ...
                    points.put("" + toMate, new ArrayList<>());
        ...
        nextButton2.setOnAction(new EventHandler<ActionEvent>() {
            ...
            public void handle(ActionEvent event) {
                try {
                    ...
                    Set<String> keys1 = points1.keySet();
                    ArrayList<String> keysList1 = new ArrayList<String>();
                    keysList1.addAll(keys1);

                    String[] st1 = new String[keysList1.size()];
                    for(int j = 0; j < keysList1.size(); j++) {
                        st1[j] = keysList1.get(j);
                    }
                    ...
                    JGraph jg_1 = new JGraph(st, "Mate");
                    for(int j = 0; j < keysList.size(); j++) {
                        for (int i = 0; i < points.get(keysList.get(j)).size(); i++) {
                            System.out.println(keysList.get(j));
                            jg_1.addToSeries("" + keysList.get(j), runs.get(i)[1], points.get(keysList.get(j)).get(i));
                        }
                    }

                    jg_1.createChart(0.0, 2000.0, 0.40, 0.80);

                    /*JGraph jg_2 = new JGraph(st1, "Temperature");
                    for(int j = 0; j < keysList1.size(); j++) {
                        for (int i = 0; i < points1.get(keysList1.get(j)).size(); i++) {
                            System.out.println(keysList1.get(j));
                            jg_2.addToSeries(""  + keysList1.get(j), runs.get(i)[1], points1.get(keysList1.get(j)).get(i));
                        }
                    }

                    jg_2.createChart(0.0, 20000.0, 0.40, 0.80);
...
//
        ...
        runGA();
        points.get("" + toMate).add(trainError);
    }
}
